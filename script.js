...
function validateLoginForm() {
    ...
    if (!studentId) {
        ...
        displayError('studentId', 'Please enter a valid Student ID (12 digits followed by 2 uppercase letters)');
        isValid = false;
    }
...
function checkLoginStatus() {
    ...
    const protectedPages = ['dashboard.html', 'program.html', 'application.html', 'payments.html', 
                           'admission-status.html', 'profile.html', 'documents.html', 
                           'transfer-requests.html', 'fees-receipts.html'];
...
function addSignature() {
    showNotification('Signature feature would open a signature pad in a real application', 'info');
}
...
function submitTransferRequest() {
    ...
    const destinationProgramme = formData.get('destinationProgramme') || document.getElementById('destinationProgramme').value;
    const destinationDepartment = formData.get('destinationDepartment') || document.getElementById('destinationDepartment').value;
    const targetLevel = formData.get('targetLevel') || document.getElementById('targetLevel').value;
    const transferSession = formData.get('transferSession') || document.getElementById('transferSession').value;
    const reason = formData.get('reason') || document.getElementById('reason').value;
    
    if (!destinationProgramme || !destinationDepartment || !targetLevel || !transferSession || !reason) {
...
function viewTransferDetails(requestId) {
    ...
    if (request) {
        showNotification(`Transfer request details: ${request.destinationProgramme} - ${request.status}`, 'info');
    }
}
...
function printReceipt(transactionId) {
    showNotification(`Generating receipt for transaction: ${transactionId}`, 'info');
    setTimeout(() => {
...
function getStamp(transactionId) {
    showNotification(`Generating stamp for transaction: ${transactionId}`, 'info');
    setTimeout(() => {
...
function editProfile(section) {
    showNotification(`Editing ${section} section. This feature would allow inline editing of profile information.`, 'info');
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function handleResponsiveLayout() {
        if (window.innerWidth <= mobileBreakpoint) {
            ...
            if (dashboardMain) {
                dashboardMain.style.marginLeft = '60px';
            }
        } else {
...
console.log('University Admission Portal JavaScript loaded successfully!');